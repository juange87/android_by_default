 ...
 *
 * @see {#link https://medium.com/@LiudasSurvila/writing-checkstyle-rules-with-android-studio-b1d31b30ca3a}
 */
...
public class AndroidDeclarationOrderCheck extends Check {
        ...
        switch (ast.getType()) {
            ...
            case TokenTypes.CTOR_DEF:
                ...
                if (state.scopeState > STATE_CTOR_DEF) {
                    if (!ignoreConstructors) {
                        log(ast, "AndroidDeclarationOrderCheck- Constructor definition in wrong order.");
                    }
                } else {
            ...
            case TokenTypes.METHOD_DEF:
                ...
                if (state.scopeState > STATE_METHOD_DEF) {
                    if (!ignoreMethods) {
                        log(ast, "AndroidDeclarationOrderCheck - Method definition in wrong order.");
                    }
                } else {
            ...
            case TokenTypes.MODIFIERS:
                if ((parentType != TokenTypes.VARIABLE_DEF) || (ast.getParent()
                                                                   .getParent()
                ...
                if (ast.findFirstToken(TokenTypes.LITERAL_STATIC) != null) {
                    ...
                    if (state.scopeState > STATE_STATIC_VARIABLE_DEF) {
                        if (!ignoreModifiers
                            ...
                            if (!ignoreParcelableCreator || !isParcelableCreator(ast)) {
                                log(ast, "AndroidDeclarationOrderCheck - Static variable definition in wrong order.");
                            }
                        }
                    } else {
                    ...
                    if (state.scopeState > STATE_INSTANCE_VARIABLE_DEF) {
                        log(ast, "AndroidDeclarationOrderCheck - Instance variable definition in wrong order.");
                ...
                if (state.declarationAccess.compareTo(access) > 0) {
                    if (!ignoreModifiers) {
                        // if it's not parcelable creator
                        if (!ignoreParcelableCreator || !isParcelableCreator(ast)) {
                            log(ast, "AndroidDeclarationOrderCheck - Variable access definition in wrong order.");
                        }
                    }
                } else {
    ...
    private boolean isParcelableCreator(DetailAST ast) {
        ...
        if (ast.getNextSibling()
               .getType() == TokenTypes.TYPE) {
            DetailAST typeAst = ast.getNextSibling();
            if (typeAst.getFirstChild()
                       .getType() == TokenTypes.DOT) {
                DetailAST dotAst = typeAst.getFirstChild();
                isParcelableType =
                        dotAst.findFirstToken(TokenTypes.IDENT) != null && dotAst.findFirstToken(TokenTypes.IDENT)
                                                                                 .getText()
                                                                                 .equals("Parcelable");
            }
        }
        if (ast.getNextSibling()
               .getNextSibling()
               .getType() == TokenTypes.IDENT) {
            DetailAST identAst = ast.getNextSibling()
                                    .getNextSibling();
            isCreatorName = identAst.getText()
                                    .equals("CREATOR");
        }
